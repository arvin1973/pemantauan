...
class M_users_peppd extends CI_Controller {
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/management/users_tim_peppd.js?v=".now("Asia/Jakarta");
                ...
                $str = $this->load->view($this->view_dir."index_peppd",$data_page,TRUE);
        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                $sql = "SELECT A.*, B.name 'nmuser' 
                            ...
                            WHERE A.active_flag!='D' AND A.group='2'";
     ...
     function add_act()
    {
        if($this->input->is_ajax_request()){
            try {
                ...
                VALUES                     ('".$userid."', '".$this->input->post("name")."', '".$this->input->post("email")."', '".md5("pancasila"."monda")."',        '2','Y',          '$current_date_time','".$this->session->userdata(SESSION_LOGIN)->userid."')";
             ...
             $tambah1="INSERT INTO tbl_user (userid,        `name`,                                      `password`,               `groupid`, `unit_code`,                        `active_flag`,`cr_dt`,             `cr_by`)
                VALUES                     ('".$userid."', '".$this->input->post("name")."', '".md5("pancasila"."monda")."',        'PPD2','','Y',          '$current_date_time','".$this->session->userdata(SESSION_LOGIN)->userid."')";
            ...
            try {
                ...
                $list_data = $this->m_ref->get_by_condition($select,$cond);
                    if($list_data->num_rows() == 0){
                        ...
                        VALUES                     ('".$iduser."', '".$this->input->post("nama")."', '".$pass."','',        'PPD2','".$this->input->post("stts")."',          '$current_date_time','".$this->session->userdata(SESSION_LOGIN)->userid."')";
    ...
    function delete(){
        if($this->input->is_ajax_request()){
            try {
                ...
                $bind1=array($userid);
                $list_data_l = $this->two_db->query($sql1,$bind1);
                if($list_data_l->num_rows() == 0){throw new Exception("Data not found, please reload this page!",0);}
                $iduser = $list_data_l->row()->userid;
